...
(function () {
  ...
  previewCanvases.forEach(canvas => {
    ...
    // Schrödinger preview: simplified 2p orbital particle cloud
    if (type === 'schrodinger') {
      // Pre-generate particles for a 2p-like orbital shape (two lobes)
      const schPts = [];
      for (let i = 0; i < 800; i++) {
        const theta = Math.random() * Math.PI;
        const phi = Math.random() * Math.PI * 2;
        const cosTheta = Math.cos(theta);
        // 2p_0 probability ~ cos²θ · r² exp(-r)
        const rMax = 8;
        const r = Math.random() * rMax;
        const prob = Math.pow(cosTheta, 2) * r * r * Math.exp(-r);
        if (Math.random() < prob * 12) {
          const x = r * Math.sin(theta) * Math.cos(phi);
          const y = r * cosTheta;
          const z = r * Math.sin(theta) * Math.sin(phi);
          schPts.push({ x, y, z, sign: cosTheta > 0 ? 1 : -1 });
        }
      }

      function drawSchPreview(t) {
        const w = canvas.width, h = canvas.height;
        const cx = w / 2, cy = h / 2;
        ctx.clearRect(0, 0, w, h);
        ...
        // Draw nucleus
        const nGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, 6);
        nGrad.addColorStop(0, '#ff5533');
        nGrad.addColorStop(1, '#ff553300');
        ctx.beginPath();
        ctx.arc(cx, cy, 6, 0, Math.PI * 2);
        ctx.fillStyle = nGrad;
        ctx.fill();

        // Draw particles (rotated)
        schPts.forEach(p => {
          ...
          ctx.fillStyle = p.sign > 0 ? 'rgba(85,136,238,0.6)' : 'rgba(238,85,102,0.6)';
          ctx.fill();
        });

        raf = requestAnimationFrame(drawSchPreview);
      }
      raf = requestAnimationFrame(drawSchPreview);
    }
